...
try {
  env = {
    ...
    // Admin panel origin when hosted on a separate subdomain (enables CORS + cross-site cookies)
...
async function createApiServer(bot) {
  ...
  // Same-origin admin: no CORS. Separate subdomain: allow ADMIN_ORIGIN only.
  const corsOrigin =
  ...
  // Cross-subdomain cookies require SameSite=None + Secure (HTTPS).
...
async function startPolling(bot) {
  ...
  try {
    ...
    log("warn", "Could not delete webhook before polling", {
...
async function main() {
  ...
  if (env.NODE_ENV === "production" && env.WEBHOOK_URL) {
    ...
    try {
      const webhookOpts = {};
      if (env.WEBHOOK_SECRET) {
        webhookOpts.secret_token = env.WEBHOOK_SECRET;
      }
      await bot.api.setWebhook(fullWebhookUrl, webhookOpts);
      log("info", "Running in webhook mode", { webhookUrl: fullWebhookUrl });
    } catch (err) {
      // Webhook host not reachable (DNS/SSL). Fall back so the bot keeps working.
      log("error", "Webhook registration failed — falling back to polling", {
